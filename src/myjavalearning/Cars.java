...
public class Cars {
	public static void main(String[] args) {
		...
		car_one.setName("Vasya");
		car_one.setManufactured(LocalDate.of(1900,01,01));
		//Запуск двигателя
		car_one.setEngineState(true);
		//Заправка
		car_one.setGasoline(500);
		System.out.printf("%s car was manufactured in %d%n", car_one.getName(),car_one.getManufactured().getYear());
		int[] speedlist = new int[] {10,2000,30,100,0,-5,-100,250};
		for(int sp:speedlist) {
			car_one.setSpeed(sp);
		
		System.out.printf("%s car moves with speed %d%n", car_one.getName(), car_one.getSpeed());
	}
	}
